...
for iter in range(num_training_batches):
    ...
    quantized_batch, perplexity, codebook_usage = vector_quantizer(data_batch)
    ...
    if (iter+1) % training_log_batches == 0:
        ...
        vq_loss_accumulator = perplexity_accumulator = 0
        used_codebook_indices_list.append(codebook_usage)
...
with open(save_address + f"codebook_usage_list_{vq_bitrate}bits_bs{batch_size}_lr{learning_rate}", "wb") as fp:
